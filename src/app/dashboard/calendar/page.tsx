  ...
  function BookingPill({ booking }: { booking: CalendarBooking }) {
    ...
    return (
      <button
        ...
        title={`${serviceLabel(booking.serviceType)} - ${booking.contact.firstName} ${booking.contact.lastName} (${booking.status})`}
        aria-label={`${serviceLabel(booking.serviceType)} with ${booking.contact.firstName} ${booking.contact.lastName}, status ${booking.status}`}
      >
        <span className="block truncate">
          {serviceLabel(booking.serviceType)}
        </span>
        <span className="block truncate opacity-75">
          {booking.contact.firstName} {booking.contact.lastName.charAt(0)}.
        </span>
      </button>
    );
  }
                ...
                {dayBookings.map((booking) => {
                  ...
                  return (
                    <button
                      ...
                      aria-label={`${serviceLabel(booking.serviceType)} with ${booking.contact.firstName} ${booking.contact.lastName}`}
                    >
